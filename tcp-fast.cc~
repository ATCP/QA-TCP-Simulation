...
FastTcpAgent::fast_calc_cwnd(double cwnd,double old_cwnd)
{
        ...
        if ( fast_opts & FAST_ALLOW_CC )
        {
			...
			double alpha=alpha_;
			double beta=beta_;
		//	printf("avgRTT:%lf baseRTT:%lf gamma:%lf alpha;%lf beta:%lf target:%lf, old:%lf, now:%lf, new:%lf\n", avgRTT_, baseRTT_, gamma, alpha, beta, (old_cwnd * (baseRTT_/avgRTT_) + alpha_), old_cwnd, cwnd, target_cwnd);
...
FastTcpAgent::recv(Packet *pkt, Handler *)
{
	...
	if (!fastrecov_) {
		...
		if ((int)tcph->seqno() > last_ack_) {
			...
			tcp_linux_state_ = 0;
			tcp_fast_est = (avgRTT_ - baseRTT_) * old_pif / avgRTT_;
